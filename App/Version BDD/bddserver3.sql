...
--
-- Table structure for table `absences_conges`
--

DROP TABLE IF EXISTS `absences_conges`;
/*!40101 SET @saved_cs_client     = @@character_set_client */;
/*!50503 SET character_set_client = utf8mb4 */;
CREATE TABLE `absences_conges` (
  `id` int NOT NULL AUTO_INCREMENT,
  `operateur_id` int NOT NULL,
  `type_absence` enum('CongePaye','RTT','SansSolde','Maladie','AccidentTravail','ArretTravail','CongeNaissance','Formation','Autorisation','Autre') NOT NULL,
  `date_debut` date NOT NULL,
  `date_fin` date NOT NULL,
  `nombre_jours` decimal(4,1) GENERATED ALWAYS AS (((to_days(`date_fin`) - to_days(`date_debut`)) + 1)) STORED,
  `statut` enum('En attente','Validé','Refusé','Annulé','En cours','Terminé') DEFAULT 'En attente',
  `source` enum('Demande','SaisieRH','Automatique') DEFAULT 'Demande',
  ...
  CONSTRAINT `fk_absences_conges_operateur` FOREIGN KEY (`operateur_id`) REFERENCES `operateurs` (`id`) ON DELETE CASCADE
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_0900_ai_ci;
/*!40101 SET character_set_client = @saved_cs_client */;

--
-- Dumping data for table `absences_conges`
--

LOCK TABLES `absences_conges` WRITE;
/*!40000 ALTER TABLE `absences_conges` DISABLE KEYS */;
/*!40000 ALTER TABLE `absences_conges` ENABLE KEYS */;
UNLOCK TABLES;
...
CREATE TABLE `contrat` (
  ...
  `numero_ordre` varchar(50) COLLATE utf8mb4_general_ci DEFAULT NULL,
...
CREATE TABLE `declaration` (
  `id` int NOT NULL AUTO_INCREMENT,
  `operateur_id` int NOT NULL,
  `type_declaration` enum('Absence','Arrêt maladie','Accident de travail','Accident de trajet') COLLATE utf8mb4_general_ci NOT NULL,
  `date_debut` date NOT NULL,
  `date_fin` date DEFAULT NULL,
  ...
  `chemin_justificatif` varchar(500) COLLATE utf8mb4_general_ci DEFAULT NULL,
  `commentaire` text COLLATE utf8mb4_general_ci,
  `lieu_accident` varchar(255) COLLATE utf8mb4_general_ci DEFAULT NULL,
  `circonstances` text COLLATE utf8mb4_general_ci,
  `temoin_nom` varchar(100) COLLATE utf8mb4_general_ci DEFAULT NULL,
  `temoin_prenom` varchar(100) COLLATE utf8mb4_general_ci DEFAULT NULL,
  `date_creation` timestamp NULL DEFAULT CURRENT_TIMESTAMP,
  `date_modification` timestamp NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP,
  PRIMARY KEY (`id`),
  KEY `idx_operateur` (`operateur_id`),
...
/*!50001 VIEW `v_conges_actifs` AS select `ac`.`id` AS `id`,`o`.`matricule` AS `matricule`,concat(`o`.`nom`,' ',`o`.`prenom`) AS `operateur`,`ac`.`type_absence` AS `type_conge`,`ac`.`date_debut` AS `date_debut`,`ac`.`date_fin` AS `date_fin`,`ac`.`nombre_jours` AS `nombre_jours`,`ac`.`statut` AS `statut`,`ac`.`created_at` AS `date_demande` from (`absences_conges` `ac` join `operateurs` `o` on((`o`.`id` = `ac`.`operateur_id`))) where ((`ac`.`statut` in ('En attente','Valid�','En cours')) and (`ac`.`date_fin` >= curdate())) order by `ac`.`date_debut` */;
...
-- Dump completed on 2025-10-30 10:46:54
